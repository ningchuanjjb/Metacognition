function X = remove_boundaries_jjb_v1(I,shifts,method,add_value)
% function X = remove_boundaries_jjb_v1(I,shifts,method,template,add_value)
...
if shifts(1)
    X(...
        (1:abs(shifts(1)))*sign(shifts(1)) + (513)*(shifts(1)<0),...
        :,...
        :...
        ) = add_value;
end
if shifts(2)
    X(:,...
        (1:abs(shifts(2)))*sign(shifts(2)) + (513)*(shifts(2)<0),...
        :...
        ) = add_value;
end
...
%             ) = add_value;
%         end
%         if shifts(2); X(:,(1:abs(shifts(2)))*sign(shifts(2)) + (513)*(shifts(2)<0),:) = add_value; end
%         if nd == 3 && shifts(3) 
