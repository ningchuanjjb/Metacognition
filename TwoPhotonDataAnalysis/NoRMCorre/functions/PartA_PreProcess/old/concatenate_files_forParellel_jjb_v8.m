function [Ycon,ln,error_flag] = concatenate_files_forParellel_jjb_v8(files,fileName,output_type)
...
elseif output_type_flag_h50_bin1 == 1
    for i = 1:numFiles
        Y_temp = h5read(files(i).name,'/mov',[1,1,1],[512,512,ln(i)]);
        %fwrite(fid,int16(Y_temp/2),'int16',0,'l');
        fwrite(fid,int16(Y_temp),'int16',0,'l');
    end    
    fclose(fid);
end
