...
plot_as         = 'angle'; % similarity, distance, or angle.  Angle will fail if distanceMetric is not 'cosine'
...
if showHist
    ...
    %end
    figure('Name',sprintf('ePAIRS :: Histogram of Distance to %.0f Nearest Neighbor, %s %s (spherical)%s',nKNN,distanceMetric,plot_as,title_suffix));
    clf; hold on;
    ...
    h = plot([1 1]*median(dists.data_mean),[histMin, histMax],'g--');
    set(h,'Color',[0.1 0.4 0.1])
    plot([1 1]*median(dists.bootstrap_mean(:)),[histMin, histMax],'r--')
