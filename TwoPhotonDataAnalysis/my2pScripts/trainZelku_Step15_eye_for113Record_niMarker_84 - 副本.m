...
searchName_reCenterGazeBias = 'reCenterGazeBias_Zelku.mat';
...
%bash % Basic adjustable parameters begin
...
[fixTolerenceTime_delay2, basic_para.fixTolerenceTime_delay2] = deal(150);%300-->50-->150-->80-->50-->150
...
[if_R_mustCorrect, basic_para.if_R_mustCorrect] = deal(1);
...
[fixWinSize_DisplayPC, basic_para.fixWinSize_DisplayPC] = deal(63);%100-->70-->140-->60-->45-->55-->63
[fixWinSize_ELHost, basic_para.fixWinSize_ELHost] = deal(63);%100-->70-->140-->60-->45-->55-->63

[fixWinSize_delay2, basic_para.fixWinSize_delay2] = deal(63);%%400-->187-->193-->225-->250-->237-->225-->63


[smallWater, basic_para.smallWater] = deal(20);%9-->6-->5-->8-->20
...
if if_221False_329True == 0
    [initGiveWaterTime, basic_para.initGiveWaterTime] = deal(170);%
elseif if_221False_329True == 1
    [initGiveWaterTime, basic_para.initGiveWaterTime] = deal(190);
...
[saccadeWater, basic_para.saccadeWater] = deal(20);%20-->10-->5-->20-->5
...
[InternalMemoryWaterRatio, basic_para.InternalMemoryWaterRatio] = deal(1.8);%1.5-->1.4-->1.32-->1.22-->1.3-->1.4
...
if if_221False_329True == 0
    ...
    [screenYpixels_bias, basic_para.screenYpixels_bias] = deal(-395);%0-->50-->0-->-50-->-140-->-125-->-75-->-150-->-190-->-170-->-160-->-240-->-280-->-255
...
[ItiTime, basic_para.ItiTime] = deal(0*randArray6 + 1000*ones(1, trial_num));%1100-->800~1200-->800~1000-->2800~3000-->1100~1300-->1000-->1500
...
[cumulativeErrorLimit, basic_para.cumulativeErrorLimit] = deal(6 * ones(1, trial_num));%6-->7-->5-->3-->2-->7
[encourageCumuErrorLimit, basic_para.encourageCumuErrorLimit] = deal(5);%5-->10-->5
...
[bgColor, basic_para.bgColor] = deal([1 1 1]*0.01);%0.25-->0.15-->0.01
...
[frameColor, basic_para.frameColor] = deal([1 1 1]*0.2);%1-->0.4-->0.8-->0.45-->0.2
[frameColor2, basic_para.frameColor] = deal([1 1 1]*0.01);%0.25-->0.15-->0.01
[frameColor3, basic_para.frameColor] = deal([1 1 1]*0.6);%1-->0.6
...
if ifSpecializedTraining_R == 1
    ...
    elseif if_221False_329True == 1
        [initGiveWaterTime, basic_para.initGiveWaterTime] = deal(250);%120
    end
    ...
    elseif if_justFix0_touch1 == 1
        [seq_length_rangeHead, basic_para.seq_length_rangeHead] = deal(1);
        [seq_length_rangeTail, basic_para.seq_length_rangeTail] = deal(4);%3
    end  
...
if ifSpecializedTraining_G == 1
    ...
    elseif if_221False_329True == 1
        [initGiveWaterTime, basic_para.initGiveWaterTime] = deal(240+30);%340-->430-->420-->300
    end
...
qKey = KbName('q');%probability of fourth frame is redFill+
...
if if_seqList == 1 && ifFormalTask == 1
    ...
    if if_length1_alone == 1
        ...
        [seq_list_oneRun, choiceCondition_flag_oneRun, randForLength4] = ...
            get_seq_list_Z6(temp_seq_length_rangeHead, seq_length_rangeTail, numFrames_rangeTail, randForLength4);
    else
        [seq_list_oneRun, choiceCondition_flag_oneRun, randForLength4] = ...
            get_seq_list_Z6(seq_length_rangeHead, seq_length_rangeTail, numFrames_rangeTail, randForLength4);
    end
        ...
        if if_length1_alone == 1 && temp_length1_flag == 0
            ...
            if temp_length1_runCount > length1_runNum
                ...
                [seq_list_oneRun, choiceCondition_flag_oneRun] = ...
                    get_seq_list_Z6(1, 1, numFrames_rangeTail); %#ok<*ASGLU>
                a = 1;
            end
        else
            [seq_list_oneRun, choiceCondition_flag_oneRun, randForLength4] = ...
...
%                        
%                        
%                        %fprintf('temptemp_t1 = %.3f ms',toc(temptemp_t0)*1000);
%                        fprintf('temptemp_t1 = %.3f ms',GetSecs*1000-temptemp_t0);
                    end                    
                else
                    flag_targetOn_marker = 0;
                    %Screen('FillOval', window, currentColorMatrices(:, currentSequence(showing_count)), showRects(:, currentSequence(showing_count)));
            ...
            if flag_targetOn_marker == 1 && last_flag_targetOn_marker == 0
                % edit in 20231204A, to reduce sample marker delay
                
                temptemp_t0 = GetSecs*1000;
                %fprintf('temptemp_t0 = %.3f ms',GetSecs*1000-temptemp_t0);
                ...
                %fprintf('temptemp_t2 = %.3f ms',toc(temptemp_t0)*1000);
                %fprintf('temptemp_t2 = %.3f ms',GetSecs*1000-temptemp_t0);
            end
  
            last_flag_targetOn_marker = flag_targetOn_marker;
...
%             end      
        ...
        if toc(t0_isTwoDelay) > twoFixationTime(trial_count)/1000 && fixOn_flag_delay2 == 1
            if if_eyeResponse == 0
                while TouchEventAvail(dev) > 0
                    TouchEventGet(dev, window);
                end
        ...
        if if_eyeResponse == 1
            ...
            for i = 1:numSquares
                ...
                if raw_eye_inside(i) == 1
                    if taskMode_noChoice_Green0_Red1 == 1 && if_R_mustCorrect == 1
                        % ignore non-target in this mode
                        a = 1;
                        %if ismember(i,currentSequence) == 0
                        %    raw_eye_inside(i) = 0;
                        %elseif ismember(i,currentSequence) == 1
...
if eye_inside(i) == 1 && sum(isFilled(i+1:end)) > 0 && isFrame(i) == 1 && ifNoBackTouch == 1
    %if taskMode_noChoice_Green0_Red1 == 0 || (taskMode_noChoice_Green0_Red1 == 1 && if_R_mustCorrect == 0)
        isCorrect(trial_count) = -1;
...
%         end
        ...
        if isTouchingDone == 1              
            ...
            if submitInside == 1
                ...
                WaitSecs(0.2);% Very important, or will lose SUBMIT marker
            end
