...
def mySubSparseryWoker(threshold_scaling,scale,mov,movu,Lyc,Lxc,max_iterations,Lyp,Lxp,gxy,percentile,sdmov,
	output1,output2,output3,output4,output5,output6,output7):

    stats,v_max,ihop,v_split,v_map = mySubSparsery(threshold_scaling,scale,mov,movu,Lyc,Lxc,max_iterations,Lyp,Lxp,gxy,percentile,sdmov)
    output1.put(len(stats))
    output2.put(stats)
    output3.put(v_max)
    output4.put(ihop)
    output5.put(v_split)
    output6.put(v_map)
    output7.put(threshold_scaling)

def mySubSparsery(threshold_scaling,scale,mov,movu,Lyc,Lxc,max_iterations,Lyp,Lxp,gxy,percentile,sdmov):
    ...
    vmultiplier = max(1, mov.shape[0] / 12000) # edited by jjb, to get more roi (towards max_iterations)
    ...
    for tj in range(max_iterations):        
        ...
        if percentile > 0:
            ...
            threshold = np.percentile(tproj, percentile) # jjb: to let percentile work actually
        else:
            threshold = Th2
        ...
        #if v_split[tj] > 1.25:
        #if v_split[tj] > 1.15: # not good, to many small roi
        if v_split[tj] > 1.35: # looks better than 1.25, 
        ...
        # added by jjb, one more time for split
        v_split[tj], ipack = two_comps(mov[:, ypix0_raw * Lxc + xpix0_raw], lam0_raw, threshold)    
    ...
    return stats,v_max,ihop,v_split,v_map
...
def sparsery(mov: np.ndarray, high_pass: int, neuropil_high_pass: int, batch_size: int, spatial_scale: int, threshold_scaling,
    ...
    mov = utils.temporal_high_pass_filter(mov=mov, width=int(high_pass)) # Memory-consuming!
 
    max_proj = mov.max(axis=0)

    sdmov = utils.standard_deviation_over_time(mov, batch_size=batch_size)

    # Very very memory-consuming!
    ...
    print('sparsery, threading0')


    threshold_scaling_initial = threshold_scaling
    temp_threshold_scaling = threshold_scaling_initial


    num_thread = 1
    temp_lowBound = 100 #800
    temp_upBound = 2000 #1400
    ...
    while True :
        ...
        done_queue5 = queue.Queue()
        done_queue6 = queue.Queue()
        done_queue7 = queue.Queue()
        ...
        for i in range(num_thread):
            ...
            # Deepcopy is necessary! Or just pointer of array, the function could ruin array
            mov_deepcopy = deepcopy(mov) 
            movu_deepcopy = deepcopy(movu)

            threading.Thread(target=mySubSparseryWoker, args=(
                temp_temp_threshold_scaling,scale,mov_deepcopy,movu_deepcopy,Lyc,Lxc,max_iterations,Lyp,Lxp,gxy,percentile,sdmov
                ,done_queue1,done_queue2,done_queue3,done_queue4,done_queue5,done_queue6,done_queue7)).start()
            del mov_deepcopy
            del movu_deepcopy     
            gc.collect()          
        print('\n',end="")
        ...
        for i in range(num_thread):
            temp_ROI_num[i] = done_queue1.get()
        ...
        for i in range(temp_betterIndex+1):
            stats = done_queue2.get()			
            v_max = done_queue3.get()	
            ihop = done_queue4.get()	
            v_split = done_queue5.get()	
            v_map = done_queue6.get()	
            temp_good_threshold_scaling = done_queue7.get()	

        done_queue1.queue.clear()
        done_queue2.queue.clear()
        done_queue3.queue.clear()
        done_queue4.queue.clear()
        done_queue5.queue.clear()
        done_queue6.queue.clear()
        done_queue7.queue.clear()

        temp_ROI_num = len(stats)
        ...
        if temp_ROI_num < temp_lowBound:
            ...
            if (temp_lowBound-temp_ROI_num) > 300:
                temp_threshold_scaling = temp_threshold_scaling - num_thread * 0.044
            elif (temp_lowBound-temp_ROI_num) > 200:
            ...
            #temp_threshold_scaling = temp_threshold_scaling + num_thread * 0.015 #0.01
            if (temp_ROI_num-temp_upBound) > 300:
                temp_threshold_scaling = temp_threshold_scaling + num_thread * 0.044
            elif (temp_ROI_num-temp_upBound) > 200:
                temp_threshold_scaling = temp_threshold_scaling + num_thread * 0.024
            ...
            print('Optimal threshold_scaling = %.3f!\n' %(temp_good_threshold_scaling),end="")
            break
