...
def get_h5_list(ops):
    ...
    if len(fs)==0:
        ...
        #print('** Found %d h5 files **'%(len(fsall)))
        #if os.path.exists('data.bin'):
        #    a=1
        #else:
        #    ops['first_tiffs'] = np.array(first_tiffs).astype('bool')
        #    print('** Converting to binary **'%(len(fsall)))
...
def find_files_open_binaries(ops1, ish5=False):
    ...
    for ops in ops1:
        ...
        if 'keep_movie_raw' in ops and ops['keep_movie_raw']:
            ...
            temp_fileName = ops['save_path'] + '/temp_data.bin'  # To break the while loop ASAP so that saving time from converting to binary          
            reg_file.append(open(temp_fileName, 'wb'))
...
def init_ops(ops):
    ...
    for j in range(0,nplanes):
        ...
        # Use this line instead of last one, so that is fast_disk path is correct.
        #fast_disk = os.path.join(ops['fast_disk'], 'plane%d'%j)
