...
def make_masks_and_enable_buttons(parent):
    ...
    parent.ops_plot['view'] = 4 # To set default view as 4 (max_projection)
...
def enable_views_and_classifier(parent):
    ...
    for b in range(len(parent.view_names)):
        ...
        #if b == 0:
        if b == 4: # To set default view as 4 (max_projection)
    ...
    for btn in parent.sizebtns.buttons():
        ...
        btn.setEnabled(True)
        #if b == 0:
        if b == 1: # To set deafult sizebtns as 'both' but not 'cells'
            btn.setChecked(True)
...
def load_proc(parent):
    ...
    if output is not None:    
        ...
        parent.setGeometry(50, 50, 1500, 800)
        parent.setWindowState(QtCore.Qt.WindowMaximized)
...
def save_mat(parent):
    ...
    hdf5storage.Options(compress=False)
    # scipy.io.savemat(matpath, {'stat': parent.stat,
    hdf5storage.savemat(matpath, {'stat': parent.stat,
                         'ops': parent.ops,
                         'F': parent.Fcell,
                         'Fneu': parent.Fneu,
                         'spks': parent.Spks,
