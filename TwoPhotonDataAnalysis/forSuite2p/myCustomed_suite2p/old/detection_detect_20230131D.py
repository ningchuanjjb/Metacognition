...
""" def myworker(aaa,output1,output2):
	output1.put(aaa)
	output2.put(aaa*2) """
...
def select_rois(ops: Dict[str, Any], mov: np.ndarray, 
	...
	if sparse_mode:
		...
		while True :
			...
			#t1 = threading.Thread(target=myworker, args=(5,done_queue1,done_queue2))	
			""" t1 = threading.Thread(target=myworker, args=(
				mov,high_pass,neuropil_high_pass,batch_size,spatial_scale,temp_threshold_scaling,max_iterations,percentile
				,done_queue1,done_queue2,done_queue3)) 														
			t1.start()
			t1.join() """

			print('Current test threshold_scaling = ',end="")
			for i in range(num_thread):
				temp_temp_threshold_scaling = temp_threshold_scaling + (i-math.floor((num_thread-1)/2))*0.01
				print('%.3f, !\n' %(temp_temp_threshold_scaling),end="")
			...
			for i in range(num_thread):
				...
				#temp_threshold_scaling = done_queue3.get()
			...
			if __name__ == '__main__':
				freeze_support()
				print('Current test threshold_scaling = %.3f!\n' %(temp_threshold_scaling),end="")
				...
				print('asd1')
				 p1 = Process(target=myworker, args=(
						mov,high_pass,neuropil_high_pass,batch_size,spatial_scale,temp_threshold_scaling,max_iterations,percentile
						,done_queue1,done_queue2)) 		
				p1 = Process(target=myworker, args=(input,done_queue1))								
				print('asd2')					
				p1.start()
				p1.join()
				print('asd3')
			...
			""" new_ops, stat = sparsedetect.sparsery(
				...
				batch_size=ops['batch_size'],
				spatial_scale=ops['spatial_scale'],
				threshold_scaling=temp_threshold_scaling,
				max_iterations=250 * ops['max_iterations'],
				percentile=ops.get('active_percentile', 0.0),
			) """
			...
			if temp_ROI_num < temp_lowBound:
				#temp_threshold_scaling = temp_threshold_scaling - 0.03 #0.05
				temp_threshold_scaling = temp_threshold_scaling - num_thread * 0.01

				""" if (temp_lowBound-temp_ROI_num) > 400:
					temp_threshold_scaling = temp_threshold_scaling - 0.05
				elif (temp_lowBound-temp_ROI_num) > 300:
					temp_threshold_scaling = temp_threshold_scaling - 0.035 #0.025
				elif (temp_lowBound-temp_ROI_num) > 200:
					temp_threshold_scaling = temp_threshold_scaling - 0.025 #0.02
				elif (temp_lowBound-temp_ROI_num) > 100:
					temp_threshold_scaling = temp_threshold_scaling - 0.015
				elif (temp_lowBound-temp_ROI_num) <= 100:
					temp_threshold_scaling = temp_threshold_scaling - 0.01 """

				
			elif temp_ROI_num > temp_upBound:
				#temp_threshold_scaling = temp_threshold_scaling + 0.03 #0.03
				temp_threshold_scaling = temp_threshold_scaling + num_thread * 0.01

				""" if (temp_ROI_num-temp_upBound) > 400:
					temp_threshold_scaling = temp_threshold_scaling + 0.05
				elif (temp_ROI_num-temp_upBound) > 300:
					temp_threshold_scaling = temp_threshold_scaling + 0.035
				elif (temp_ROI_num-temp_upBound) > 200:
					temp_threshold_scaling = temp_threshold_scaling + 0.025
