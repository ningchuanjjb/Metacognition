...
temp_dir = os.path.split(current_rootPath)
currentSession = temp_dir[1]
print('currentSession is %s\n' %(currentSession),end="")
...
dataBase_path = 'C:/ASDROOT/STUDY/twoPhotonData_motionCorrected_temporary/' + currentSession + '/' + target_dir
print('dataBase_path is %s\n' %(dataBase_path),end="")
...
#ops['fs'] = 30.0 # sampling rate (PER PLANE e.g. for 12 plane recordings it will be around 2.5), 15.0 --> 30.0
...
db = {
      ...
      'data_path': [dataBase_path], # a list of folders with tiffs 
                                             # (or folder of folders with tiffs if look_one_level_down is True, or subfolders is not empty)                                            
      'subfolders': [], # choose subfolders of 'data_path' to look in (optional)
      'fast_disk': dataBase_path, # string which specifies where the binary file will be stored (should be an SSD)
