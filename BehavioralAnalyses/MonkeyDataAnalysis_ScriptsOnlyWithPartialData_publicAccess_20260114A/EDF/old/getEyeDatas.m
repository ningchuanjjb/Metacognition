...
%             end
   ...
   %% collect eye data for pupil size    
    if no_choice_version ~= 1
        triali = decision_making(:,2)';
%         triali = size(decision_making,1);
    else
        triali = 1:size(no_choice,1); %% for no choice version
    end
    for j = triali(~isnan(triali))
        trialST = find(ndata(:,1) >= stimulus_term(stimulus_term(:,2) == j,1),1); % start point
        trialST = trialST; % 改动前后多少ms换这个数字
        if isempty(decision_making)
            trialEND = find(ndata(:,1) <= no_choice(no_choice(:,2) == j,1),1,'last'); % for no choice version
        elseif isnan(decision_making(decision_making(:,2) == j))
%             trialEND = find(ndata(:,1) <= no_choice(no_choice(:,2) == j,1),1,'last'); % if no choice made
            skipped_trial_num = skipped_trial_num + 1;
            continue
        else
            trialEND = find(ndata(:,1) <= trial_end(trial_end(:,2) == (j),1),1,'last'); % end point
%              trialEND = find(ndata(:,1) <= decision_making(decision_making(:,2) == j,1),1,'last'); % end point
            trialEND = trialEND ; % 改动前后多少ms换这个数字
        end
        pupil_eyedata{j} = ndata(trialST:trialEND,:);
        
        %% collect eye data for saccade
        saccadeST = find(ndata(:,1) >= stimulus_on(stimulus_on(:,2) == j,1),1); % start point
        saccadeST= saccadeST; % 改动前后多少ms换这个数字
        if isempty(decision_making)
            saccadeEND = find(ndata(:,1) <= no_choice(no_choice(:,2) == j,1),1,'last'); % if no choice version
        elseif isnan(decision_making(decision_making(:,2) == j))
%             saccadeEND = find(ndata(:,1) <= no_choice(no_choice(:,2) == j,1),1,'last'); % if no choice made
            skipped_trial_num = skipped_trial_num + 1;
           continue
        else
%             saccadeEND = find(ndata(:,1) <= start_choice(start_choice(:,2) == j,1),1,'last'); % end point
     saccadeEND = find(ndata(:,1) <= trial_end(trial_end(:,2) == (j),1),1,'last');
            saccadeEND = saccadeEND; % 改动前后多少ms换这个数字
        end
        ...
        [baseset_purified,~,errorflagj,purifyNUMj] = BlinkNoisePurify_NaN(baseset,4,dt,error_set); % remove the blink data, replaced by NaN
