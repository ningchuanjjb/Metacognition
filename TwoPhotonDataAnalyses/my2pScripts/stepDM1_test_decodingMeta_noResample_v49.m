...
% if_dff_singleSession1_twoSession2_allSession3 = 2;
%
% if if_monkey_D0_Z1 == 0
%     currentSessionIndex_AB = 8;
...
%     %a1 = sum(choiceMemoryBoolIndex);
%     a1 = sum(choiceOffloadBoolIndex);
%     a2 = sum(choiceBoolIndex);
%
%     metaDecoderPrctileThreshold = a1/a2;
%
% else
%     metaDecoderPrctileThreshold = 0.5;
% end
...
if if_compute_train == 1
    ...
    %svm_options.metaDecoderPrctileThreshold = metaDecoderPrctileThreshold;
    
    
    x_raw = F_dff_decisionBin1';
    x = x_raw(choiceBoolIndex_validLength,:);
    %y_raw = choiceMemoryBoolIndex';
    %y = y_raw(choiceBoolIndex_validLength);
...
% metaDecoderThreshold = prctile(svm_Meta.Posterior_2d_n11n,metaDecoderPrctileThreshold*100);

%%
if if_prctile_metaDecoderThreshold == 1
    temp_thresholdRange = 0.01:0.001:0.99;
    
    AUROC_multi = zeros(1,length(temp_thresholdRange));
    ...
    for temptempi=1:length(temp_thresholdRange)
        ...
        for tempi=1:3
            ...
            temptempBoolIndex = ismember(seqIndex_choice_validLength,temp_range)';
            
            if tempi == 1
                predictLabel_length1 = predictLabel(temptempBoolIndex);
                trueLabel_length1 = trueLabel(temptempBoolIndex);
            elseif tempi == 2
                predictLabel_length2 = predictLabel(temptempBoolIndex);
                trueLabel_length2 = trueLabel(temptempBoolIndex);
            elseif tempi == 3
                predictLabel_length3 = predictLabel(temptempBoolIndex);
                trueLabel_length3 = trueLabel(temptempBoolIndex);
            end
        end
        svm_choiceMemory_length1_hit = sum(predictLabel_length1(trueLabel_length1==true))/sum(trueLabel_length1==true);
        svm_choiceMemory_length1_falseAlarm = sum(predictLabel_length1(trueLabel_length1==false))/sum(trueLabel_length1==false);
        
        svm_choiceMemory_length2_hit = sum(predictLabel_length2(trueLabel_length2==true))/sum(trueLabel_length2==true);
        svm_choiceMemory_length2_falseAlarm = sum(predictLabel_length2(trueLabel_length2==false))/sum(trueLabel_length2==false);
        
        svm_choiceMemory_length3_hit = sum(predictLabel_length3(trueLabel_length3==true))/sum(trueLabel_length3==true);
        svm_choiceMemory_length3_falseAlarm = sum(predictLabel_length3(trueLabel_length3==false))/sum(trueLabel_length3==false);
        
        %fprintf('metaDecoder Hit = %.3f (%.3f,%.3f,%.3f).\n',svm_choiceMemory_hit,svm_choiceMemory_length1_hit,svm_choiceMemory_length2_hit,svm_choiceMemory_length3_hit);
        %fprintf('metaDecoder FA  = %.3f (%.3f,%.3f,%.3f).\n',svm_choiceMemory_falseAlarm,svm_choiceMemory_length1_falseAlarm,svm_choiceMemory_length2_falseAlarm,svm_choiceMemory_length3_falseAlarm);
        
        svm_choiceMemory_lengthx_roc = ...
            [svm_choiceMemory_length1_hit,svm_choiceMemory_length2_hit,svm_choiceMemory_length3_hit;...
            svm_choiceMemory_length1_falseAlarm,svm_choiceMemory_length2_falseAlarm,svm_choiceMemory_length3_falseAlarm];
        
        [M,I] = sort(svm_choiceMemory_lengthx_roc(2,:),'ascend'); %#ok<*ASGLU>
        svm_choiceMemory_lengthx_roc_sorted = svm_choiceMemory_lengthx_roc(:,I);
        
        x = [0 svm_choiceMemory_lengthx_roc_sorted(2,:) 1];
        y = [0 svm_choiceMemory_lengthx_roc_sorted(1,:) 1];
        % x = [0 0 1];
        % y = [0 1 1];
        AUROC = trapz(x,y);
        
        AUROC_multi(temptempi) = AUROC;
    end
...
if if_compute_train == 1
    if if_compute_shuffle == 1
        ...
        parfor shuffleCount=1:shuffleNum
            ...
            svm_options = struct;
            svm_options.numSeq = numSeq;
            svm_options.seqIndex_choice_validLength = seqIndex_choice_validLength;
            svm_options.KFold_num = KFold_num_meta;
            svm_options.t_decoder = t_decoder;
            %svm_options.metaDecoderPrctileThreshold = metaDecoderPrctileThreshold;
            ...
            x = x_raw(choiceBoolIndex_validLength,:);
            % y_raw = choiceMemoryBoolIndex';
            y_raw = choiceMemoryBoolIndex_shuffled';
            y = y_raw(choiceBoolIndex);
            
            
            svm_Meta_shuffled = fun_metaDecoder(x,y,svm_options); %#ok<*PFBNS>
            
            %% Get AUROC_shuffled
            %predictLabel_shuffled = svm_Meta_shuffled.Posterior_2d_n11n > 0.5;
            
            %temp_threshold_shuffled = prctile(svm_Meta_shuffled.Posterior_2d_n11n,metaDecoderPrctileThreshold*100);
            %predictLabel_shuffled = svm_Meta_shuffled.Posterior_2d_n11n > temp_threshold_shuffled;%0.5
            
            predictLabel_shuffled = svm_Meta_shuffled.Posterior_2d_n11n > metaDecoderThreshold;
            
            trueLabel_shuffled = svm_Meta_shuffled.temp_svm_Y_valid_T;
            
            svm_choiceMemory_hit_shuffled = sum(predictLabel_shuffled(trueLabel_shuffled==true))/sum(trueLabel_shuffled==true);
            svm_choiceMemory_falseAlarm_shuffled = sum(predictLabel_shuffled(trueLabel_shuffled==false))/sum(trueLabel_shuffled==false);
            ...
            for tempi=1:3
                ...
                if tempi == 1
                    predictLabel_shuffled_length1 = predictLabel_shuffled(temptempBoolIndex);
                    trueLabel_shuffled_length1 = trueLabel_shuffled(temptempBoolIndex);
                elseif tempi == 2
                    predictLabel_shuffled_length2 = predictLabel_shuffled(temptempBoolIndex);
                    trueLabel_shuffled_length2 = trueLabel_shuffled(temptempBoolIndex);
                elseif tempi == 3
                    predictLabel_shuffled_length3 = predictLabel_shuffled(temptempBoolIndex);
                    trueLabel_shuffled_length3 = trueLabel_shuffled(temptempBoolIndex);
                end
            end
            svm_choiceMemory_length1_hit_shuffled = sum(predictLabel_shuffled_length1(trueLabel_shuffled_length1==true))/sum(trueLabel_shuffled_length1==true);
            svm_choiceMemory_length1_falseAlarm_shuffled = sum(predictLabel_shuffled_length1(trueLabel_shuffled_length1==false))/sum(trueLabel_shuffled_length1==false);
            
            svm_choiceMemory_length2_hit_shuffled = sum(predictLabel_shuffled_length2(trueLabel_shuffled_length2==true))/sum(trueLabel_shuffled_length2==true);
            svm_choiceMemory_length2_falseAlarm_shuffled = sum(predictLabel_shuffled_length2(trueLabel_shuffled_length2==false))/sum(trueLabel_shuffled_length2==false);
            
            svm_choiceMemory_length3_hit_shuffled = sum(predictLabel_shuffled_length3(trueLabel_shuffled_length3==true))/sum(trueLabel_shuffled_length3==true);
            svm_choiceMemory_length3_falseAlarm_shuffled = sum(predictLabel_shuffled_length3(trueLabel_shuffled_length3==false))/sum(trueLabel_shuffled_length3==false);
            
            svm_choiceMemory_lengthx_roc_shuffled = ...
                [svm_choiceMemory_length1_hit_shuffled,svm_choiceMemory_length2_hit_shuffled,svm_choiceMemory_length3_hit_shuffled;...
                svm_choiceMemory_length1_falseAlarm_shuffled,svm_choiceMemory_length2_falseAlarm_shuffled,svm_choiceMemory_length3_falseAlarm_shuffled];
            
            [M,I] = sort(svm_choiceMemory_lengthx_roc_shuffled(2,:),'ascend');
            svm_choiceMemory_lengthx_roc_sorted_shuffled = svm_choiceMemory_lengthx_roc_shuffled(:,I);
            
            x = [0 svm_choiceMemory_lengthx_roc_sorted_shuffled(2,:) 1];
            y = [0 svm_choiceMemory_lengthx_roc_sorted_shuffled(1,:) 1];
            % x = [0 0 1];
            % y = [0 1 1];
    ...
    elseif if_compute_shuffle == 0
        %AUROC_shuffled_prctile = 0.504;
        %AUROC_shuffled_prctile = 0.536;
        if if_trainMeta_0baseline_1delay1 == 1
            AUROC_shuffled_prctile = 0.505;
        elseif if_trainMeta_0baseline_1delay1 == 0
            AUROC_shuffled_prctile = 0.513;
        end
    end
end
...
temp_meta_trialLevel = zeros(length(temp_seqIndex),1);
% temp_meta_trialLevel(choiceBoolIndex) = meta_trialLevel_choice;
% temp_meta_trialLevel(~choiceBoolIndex) = meta_trialLevel_noChoice;
...
for tempi=1:sum(numSeq(1:3))
    ...
    meta_seqLevel_choice(tempi) = mean(temp_meta_trialLevel(temptempBoolIndex_choice));
    meta_seqLevel_choiceMemory(tempi) = mean(temp_meta_trialLevel(temptempBoolIndex_choiceMemory));
    meta_seqLevel_choiceOffload(tempi) = mean(temp_meta_trialLevel(temptempBoolIndex_choiceOffload));
    meta_seqLevel_noChoice(tempi) = mean(temp_meta_trialLevel(temptempBoolIndex_noChoice));
    meta_seqLevel_all(tempi) = mean(temp_meta_trialLevel(temptempBoolIndex));
end
...
if if_plot == 1
    ...
    %set(gcf,'Position',[10 50 960 240]);
    %set(gcf,'Position',[10 50 720 240*0.9]);
    %set(gcf,'Position',[10 50 720 240*0.9*0.98]);
    ...
    % predictLabel = svm_Meta.Posterior_2d_n11n > 0.5;%0.5
    % predictLabel = svm_Meta.Posterior_2d_n11n > 0.65;%0.5
    
    % temp_threshold = prctile(svm_Meta.Posterior_2d_n11n,metaDecoderPrctileThreshold*100);
    % predictLabel = svm_Meta.Posterior_2d_n11n > temp_threshold;%0.5
    predictLabel = svm_Meta.Posterior_2d_n11n > metaDecoderThreshold;%0.5
    ...
    [M,I] = sort(svm_choiceMemory_lengthx_roc(2,:),'ascend');
    svm_choiceMemory_lengthx_roc_sorted = svm_choiceMemory_lengthx_roc(:,I);
    ...
    % plot(x,y,'--o', 'MarkerSize', 10, 'MarkerFaceColor','#0072BD');
    plot(x,y,'--o','MarkerSize',tempSize,'color',[0.25 0.25 0.25],...
        'MarkerFaceColor',[0.25 0.25 0.25],'MarkerEdgeColor',[0.25,0.25,0.25]);
    hold on
    plot(0:1,0:1,'--','Color','k');
    
    % h = fill([x,1:-1:0],[y,0 0],[0.4660 0.6740 0.1880]);
    ...
    set(gca,'box','off');
    % xlabel('False Alarm Rate (memory|offload)', 'FontSize', 12, 'FontWeight', 'bold');
    % ylabel('Hit Rate (memory|memory)', 'FontSize', 12, 'FontWeight', 'bold');
    % xlabel({'False Alarm Rate';'memory|offload'}, 'FontSize', 12, 'FontWeight', 'bold');
    % ylabel({'Hit Rate';'memory|memory'}, 'FontSize', 12, 'FontWeight', 'bold');
    xlabel({'False Alarm Rate'}, 'FontSize', 10, 'FontWeight', 'normal');
    ylabel({'Hit Rate'}, 'FontSize', 10, 'FontWeight', 'normal');
    
    %text(0.285,0.25,sprintf('AUROC=%.3f',AUROC),'fontsize',9,'FontWeight','normal');
    %text(0.285,0.10,sprintf('(95%%=%.3f)',AUROC_shuffled_prctile),'fontsize',9,'FontWeight','normal');
    text(0.290,0.25,sprintf('AUROC=%.3f',AUROC),'fontsize',8,'FontWeight','normal');
    text(0.290,0.10,sprintf('(95%%=%.3f)',AUROC_shuffled_prctile),'fontsize',8,'FontWeight','normal');
    
    
    % title(sprintf('ROC curve, meta-memory docoder \n AUROC = %.3f, Decoding accuracy = %.3f',...
    %     AUROC,svm_Meta.svm_posterior_allTrial_length123), 'FontSize', 14, 'FontWeight', 'bold');
    % title(sprintf('ROC curve, meta-memory docoder \n AUROC = %.3f (95%% = %.3f) \n Decoding accuracy = %.3f',...
    %     AUROC,AUROC_shuffled_prctile,svm_Meta.svm_posterior_allTrial_length123), 'FontSize', 14, 'FontWeight', 'bold');
    % title(sprintf('ROC curve, AUROC = %.3f (95%% = %.3f) \n Decoding accuracy = %.3f',...
    %     AUROC,AUROC_shuffled_prctile,svm_Meta.svm_posterior_allTrial_length123), 'FontSize', 14, 'FontWeight', 'bold');
    % title(sprintf('AUROC = %.3f (95%% = %.3f) \n Decoding accuracy = %.3f',...
    %     AUROC,AUROC_shuffled_prctile,svm_Meta.svm_posterior_allTrial_length123), 'FontSize', 12, 'FontWeight', 'bold');
    %title(sprintf(' AUROC=%.3f \n (95%%=%.3f) ',...
    %   AUROC,AUROC_shuffled_prctile), 'FontSize', 10, 'FontWeight', 'normal');
    title(sprintf('AUROC'), 'FontSize', 10, 'FontWeight', 'normal');
    ...
    for loopCount=1:2
        ...
        set(gca, 'FontSize', 10)
        set(gca,'box','off');% 取消右、上边框
        xlabel(sprintf('Meta-memory'), 'FontSize', 10, 'FontWeight', 'normal');
        ...
        if if_trainMeta_0baseline_1delay1 == 1
            text(0.60,0.92,sprintf('r=%.3f',r), 'fontsize',9,'FontWeight','normal');
            text(0.60,0.79,sprintf('%s',tempTxt), 'fontsize',9,'FontWeight','normal');
        elseif if_trainMeta_0baseline_1delay1 == 0
            text(0.10,0.32,sprintf('r=%.3f',r), 'fontsize',9,'FontWeight','normal');
            text(0.10,0.19,sprintf('%s',tempTxt), 'fontsize',9,'FontWeight','normal');
        end
        ...
        temp_title = title(sprintf('%s',tempStr), 'FontSize', 10, 'FontWeight', 'normal');
        temp_title.Interpreter = 'none';
    ...
    text(0.55,0.25,sprintf('r=%.3f',r), 'fontsize',9,'FontWeight','normal');
    text(0.55,0.12,sprintf('%s',tempTxt), 'fontsize',9,'FontWeight','normal');
    ...
    elseif if_memoryPrecision_accuracy0_sigma1 == 1
        temp_title = title(sprintf('Neuronal correlation'), 'FontSize', 10, 'FontWeight', 'normal');
    end
    temp_title.Interpreter = 'none';
    ...
    if true
        ...
        plot([1 1].*metaDecoderThreshold,[y_min-(y_max-y_min)*0.1 AUROC_optimal],':','color',[0.25 0.25 0.25],'linewidth',1);
        hold on
        
        plot([x_min-(x_max-x_min)*0.05 metaDecoderThreshold],[1 1].*AUROC_optimal,':','color',[0.25 0.25 0.25],'linewidth',1);
        hold on
        ...
        set(gca, 'FontSize', 8)
        set(gca,'box','off');% 取消右、上边框
        xlabel('Decision boundary', 'FontSize', 12, 'FontWeight', 'bold');
        ylabel('AUROC', 'FontSize', 12, 'FontWeight', 'bold');
        temp_title = title(sprintf('Meta-memory decoder'),...
