% Chuan's 4th script (20251214)
% This script: To train and test the location working memory decoder, related to figure 2.
% clear
close all

%% Initialization for demo data

if_loadDemoData = 1;% only for run demo data
...
if_resample_seqBased0_locBased1 = 1;%1
...
if_memoryPrecision_accuracy0_sigma1 = 1;%0,1
...
%     currentSessionIndex_AB = 2;%S06B
    ...
    currentSessionIndex_AB = 9;%S10B, good (Fig. 2 & 3 & 5)
    
%     currentSessionIndex_AB = 10;%S06C
%     currentSessionIndex_AB = 13;%S06RA, good (Fig. 4)
...
elseif if_loadDemoData == 1
    s = load([demoData_dirPath,'\','FittingResults_CCM_noOrder_distri_kwp0_v13_loopN11N_onlyDiagonal_allMemory_20230907A']);
end
...
if if_dff_singleSession1_twoSession2_allSession3 < 3
    ...
    elseif if_loadDemoData == 1
        neuronBoolIndex = true(size(F_dff_decisionBin_demo,1),1); %#ok<*PREALL>
    end
