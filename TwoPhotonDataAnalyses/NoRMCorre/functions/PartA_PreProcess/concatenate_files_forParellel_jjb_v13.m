function [Ycon,ln,error_flag] = concatenate_files_forParellel_jjb_v13(files,fileName,output_type,if_delete_partialFile)
...
if(~exist('if_delete_partialFile','var'))
    if_delete_partialFile = 0; %#ok<*NASGU>
end
...
elseif strcmp(output_type,'suite2pbin') == 1
    ...
    [parts_pathstr,parts_fname,~] = fileparts(fileName);
    fileName = fullfile(parts_pathstr,[parts_fname,'.bin']);
    %fileName = 'data.bin';
end
...
if output_type_flag_h50_bin1 == 0
    ...
    %h5create(fileName,'/mov',[sizY,If],'Chunksize',[sizY,1],'Datatype',data_type,'Deflate',1,'Shuffle',true);
elseif output_type_flag_h50_bin1 == 1
...
elseif output_type_flag_h50_bin1 == 1
    ...
    for i = 1:numFiles
        temp_lni = ln(i);
        temp_lnijRange = zeros(batchNum,2);
        step_temp_lnij = floor(temp_lni/batchNum);
        for tempj=1:batchNum
            if tempj == 1
                temp_lnijRange(tempj,1) = 1;
                temp_lnijRange(tempj,2) = temp_lnijRange(tempj,1)+step_temp_lnij-1;
            elseif tempj < batchNum
                temp_lnijRange(tempj,1) = temp_lnijRange(tempj-1,2)+1;
                temp_lnijRange(tempj,2) = temp_lnijRange(tempj,1)+step_temp_lnij-1;
            elseif tempj == batchNum
                temp_lnijRange(tempj,1) = temp_lnijRange(tempj-1,2)+1;
                temp_lnijRange(tempj,2) = temp_lni;
            end
        end
        a = 1;
        ...
        for tempj=1:batchNum
            a = 1;
            %Y_temp = int16(h5read(files(i).name,'/mov',[1,1,1],[512,512,ln(i)]));
            current_step = temp_lnijRange(tempj,2)-temp_lnijRange(tempj,1)+1;
            Y_temp = int16(h5read(files(i).name,'/mov',[1,1,temp_lnijRange(tempj,1)],[512,512,current_step]));
            fwrite(fid,Y_temp,'int16',0,'l');
            clear Y_temp
            a = 1;
        end
