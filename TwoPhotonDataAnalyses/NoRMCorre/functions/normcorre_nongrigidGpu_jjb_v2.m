function [M_final,shifts,template,options,col_shift] = normcorre_nongrigidGpu_jjb_v2(Y,options,template)
...
if options.correct_bidir && isempty(options.col_shift)
    col_shift = correct_bidirectional_offset(gather(Y),options.nFrames,options.bidir_us);
...
switch lower(options.output_type)
    case 'mat'
        M_final = single(zeros([sizY,T],data_type));
    case 'memmap'
                ...
                for j = 1:length(yy_s)           
                    for k = 1:length(zz_s)
                        if nd == 2
                            ...
                            %[output,Greg] = dftregistration_min_max_gpu_jjb_v11_forNonrigid_v1(fftTemp{i,j,k},fftY{i,j,k},us_fac,lb-max_dev(1:2),ub+max_dev(1:2),options.phase_flag);                              
                            
                 
buf1ft = fftTemp{i,j,k};
buf2ft = fftY{i,j,k};
usfac = us_fac;
...
FTpad_coeff = 2;
buf_pad = FTpad_gpu_jjb_v4_forNonrigid_v1(buf_prod,FTpad_coeff*[d1 d2]);
...
Nr2 = ifftshift(gpuArray(-fix(d1):ceil(d1)-1));
Nc2 = ifftshift(gpuArray(-fix(d2):ceil(d2)-1));

if Nr2(row_shift)/FTpad_coeff > temp_max_shift(1) || Nc2(col_shift)/FTpad_coeff > temp_max_shift(2) || Nr2(row_shift)/FTpad_coeff < temp_min_shift(1) || Nc2(col_shift)/FTpad_coeff < temp_min_shift(2)
    CCabs2 = CCabs;
    CCabs2(Nr2/FTpad_coeff>temp_max_shift(1),:) = 0;
    CCabs2(:,Nc2/FTpad_coeff>temp_max_shift(2)) = 0;
    CCabs2(Nr2/FTpad_coeff<temp_min_shift(1),:) = 0;
    CCabs2(:,Nc2/FTpad_coeff<temp_min_shift(2)) = 0;
    [row_shift, col_shift] = find(CCabs == max(CCabs2(:)),1,'first');
end
CCmax = CC(row_shift,col_shift)*d1*d2;
...
temp_Nr = ifftshift(-fix(d1/2):ceil(d1/2)-1);
temp_Nc = ifftshift(-fix(d2/2):ceil(d2/2)-1);
[temp_Nc,temp_Nr] = meshgrid(temp_Nc,temp_Nr);
Greg = buf2ft.*exp(1i*2*pi*(-row_shift*temp_Nr/d1-col_shift*temp_Nc/d2));
Greg = Greg*exp(1i*diffphase);
                        ...
                        M_temp = remove_boundaries(gather(M_temp),gather(output(3:end)),'copy',gather(template{i,j,k}));
        ...
        switch lower(options.shifts_method)
            case 'fft'
                ...
                if flag_interp    
                    ...
                    Mf = cell2mat_ov_gpu_jjb_v1(M_fin,xx_us,xx_uf,yy_us,yy_uf,zz_us,zz_uf,overlap_post,sizY) - add_value;                    
                end                             
            ...
            otherwise
                ...
                if nd == 3                
                    ...
                    Mf = imwarp(gather(Yt),-cat(3,shifts_up(:,:,2),shifts_up(:,:,1)),options.shifts_method,'FillValues',fill_value);                      
                end    
...
if it == iter
    ...
    template = cell2mat_ov_gpu_jjb_v1(template,xx_s,xx_f,yy_s,yy_f,zz_s,zz_f,overlap_pre,sizY);    
end
if memmap
