function I = shift_reconstruct_jjb_v1(Y,shifts,diffphase,us_fac,Nr,Nc,Np,method,add_value)
...
%     if nargin < 7 || isempty(Np); Np = ifftshift(-fix(np/2):ceil(np/2)-1); end
    ...
    %if us_fac > 0
        ...
        %Greg = buf2ft.*exp(1i*2*pi*(-shifts(1)*Nr/512-shifts(2)*Nc/512));
    %elseif us_fac == 0
    %    Greg = buf2ft;
    %end
    %Greg = Greg*exp(1i*diffphase);
    
    Greg = Y.*exp(1i*2*pi*(-shifts(1)*Nr/512-shifts(2)*Nc/512)+1i*diffphase);
    I = real(ifftn(Greg));
    I = remove_boundaries_jjb_v1(I,shifts,method,add_value);    
% else
%     if isreal(Y)
%         I = Y;
%     else
