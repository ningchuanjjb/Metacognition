...
def main():
    ...
    if args.version:
        ...
        # Try to find initial_statfile
        current_path = os.getcwd()
        print('current_path is %s\n' %(current_path),end="")
        stat_fileName_prefix = 'stat.npy'
    
        file_list = glob.glob((current_path+'/'+stat_fileName_prefix+'*'))        
        ...
        if len(file_list) > 0:        
            ...
            #initial_statfile = target_fullDir[1]
            initial_statfile = file_list[len(file_list)-1]
            print('Choose %s to analyse!\n' %(initial_statfile),end="")

            gui.run(initial_statfile)
        else:
            gui.run()
