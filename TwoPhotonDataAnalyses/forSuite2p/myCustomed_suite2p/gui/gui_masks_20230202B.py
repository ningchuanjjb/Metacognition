...
def init_masks(parent):
    ...
    for n in np.arange(ncells-1,-1,-1,int):
        ...
        if ypix is not None and not iignore[n]:
            ...
            xpix = stat[n]['xpix']
            lam = stat[n]['lam']
            #lam = lam / lam.sum() #comment here to fix show bug
            i = int(1-iscell[n])
            # add cell on top
...
def draw_masks(parent): #ops, stat, ops_plot, iscell, ichosen):
    ...
    if view==0:
        ...
        # Use colored ROI instead of colored circle as mask
        for n in parent.imerge:
            ypix = parent.stat[n]['ypix'].flatten()
            xpix = parent.stat[n]['xpix'].flatten()
            col = parent.colors['cols'][color,n]
            M[wplot] = make_chosen_coloredROI_jjb(M[wplot], ypix, xpix, col)      
...
def make_chosen_coloredROI_jjb(M0, ypix, xpix, col):
