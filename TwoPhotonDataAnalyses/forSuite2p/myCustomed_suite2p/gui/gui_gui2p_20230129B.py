...
class MainWindow(QMainWindow):
        ...
        if self.loaded:
            if event.modifiers() != QtCore.Qt.ControlModifier and event.modifiers() != QtCore.Qt.ShiftModifier:
                ...
                ##Agus
                    ...
                    # To flip between views of p1 and p2, and then choose the next roi
                    self.ROI_remove()
                    ctype = ~self.iscell[self.ichosen]
                    #print(ctype)
    ...
    def ichosen_stats(self):
        ...
        # Add probability of cell in stat info
        for k in range(1, len(self.stats_to_show) + 1):            
            if k == len(self.stats_to_show):
                key = self.stats_to_show[k - 1]
                ival = self.probcell[n]
                self.ROIstats[k].setText(key + ": %.2f" % (ival))
            else:
                key = self.stats_to_show[k - 1]
                ival = self.stat[n][key]
                if k == 1:
